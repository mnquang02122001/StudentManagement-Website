...
function genClassName(classType) {
  ...
  const lClassName = isTypeOne
    ? lastCLCClassName[rand(3)]
    : lastTClassName[rand(3)];
...
async function generateClasses(quantity) {
  ...
  while (quantity != 0 && !errorCheck) {
    ...
    let studentIdRange = studentIds.slice(startStudentId, endStudentId);
  ...
  errorCheck
    ...
    : console.log('Imported classes data successfully!');
}
